...
class BertForSequenceClassificationCNN(PreTrainedBertModel):

    def __init__(self, config, num_labels=2):
        super(BertForSequenceClassificationCNN, self).__init__(config)
        static = True

        self.num_labels = num_labels
        self.kernel_num = 3
        self.kernel_sizes = [16, 32, 64]
        self.num_labels = num_labels
        self.bert = BertModel(config)
        ...
        self.static = static
        self.convs1 = nn.ModuleList([nn.Conv2d(1, self.kernel_num, (K, config.hidden_size)) for K in self.kernel_sizes])
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.fc1 = nn.Linear(len(self.kernel_sizes) * self.kernel_num, self.num_labels)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        output_all_encoded_layers, pooled_output = self.bert(input_ids, token_type_ids, attention_mask,
                                                             output_all_encoded_layers=False)
        if self.static:
            output_all_encoded_layers = Variable(output_all_encoded_layers)

        output_all_encoded_layers = output_all_encoded_layers.unsqueeze(1)  # (N, Ci, W, D)

        x = [F.relu(conv(output_all_encoded_layers)).squeeze(3) for conv in self.convs1]
